...
import { challengeService, Challenge } from '@/services/challenge_service';
...
export function ChallengeDashboard({ userId }: Props) {
    ...
    const loadData = async () => {
        try {
            ...
            const [currentChallenges, userChallengesData] = await Promise.all([
                challengeService.getCurrentChallenges(),
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {challenges.map((challenge) => {
                            ...
                            const isCompleted = userChallenge?.completedBy?.includes(userId);
                                                ...
                                                <div className="mt-4 flex items-center justify-between">
                                                    <div className="flex items-center space-x-2">
                                                        ...
                                                        <span className="text-sm">
                                                            {t(`ChallengeDashboard.reward.${challenge.reward.type}`)}: {challenge.reward.value}
                                                        </span>
                                                    </div>
                                                    <div className="flex items-center space-x-2">
