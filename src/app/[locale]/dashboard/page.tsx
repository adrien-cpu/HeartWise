...
export default function LocalizedDashboardPage() {
  ...
  useEffect(() => {
    ...
    fetchProfileData();
  }, [user, authLoading, router, t, tProfile, toast]);
  ...
  if (authLoading || (loadingProfileData && !profile && user)) {
    return (
        ...
        {loadingProfileData && !profile ? (
          ...
          <div className="flex items-center space-x-4">
            <Avatar className="h-16 w-16 border" data-ai-hint={profile.dataAiHint || "user"}>
              <AvatarImage src={profile.profilePicture || undefined} alt={profile.name || user.displayName || t('userAlt')} />
              <AvatarFallback className="text-2xl">{getInitials(profile.name || user.displayName)}</AvatarFallback>
            </Avatar>
            <div>
              <h1 className="text-3xl font-bold">{t('welcome', { name: profile.name || user.displayName || t('userAlt') })}</h1>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold">{t('welcome', { name: user.displayName || t('userAlt') })}</h1>
            ...
            {loadingProfileData ? (
              ...
              <div className="flex flex-wrap gap-2">
                {profile.rewards.slice(0, 4).map(reward => (
                  <Badge key={reward.id} variant="secondary" className="flex items-center gap-1.5 p-1.5 pr-2.5 text-xs cursor-help" title={`${tRewards(`badge_${reward.type}_name`)}: ${tRewards(`badge_${reward.type}_desc`)}`}>
